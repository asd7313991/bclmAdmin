...
public class TokenUtil {
	...
	public static  void  generateToken(int id,HttpServletRequest req, HttpServletResponse resp) {
		...
		for (int i = 0; i < cs.length; i++) {
			int c = cs[i];
			result.append((char) (97 + (c + (i % 2 == 0 ? i * 2 : i + 3)) % 26));
			result.append((char) (Math.random() * 26 + 97));
			result.append((char) (Math.random() * 26 + 97));
		}
		result.insert(0, Integer.toString((int) (Math.random() * 36), 36) + Integer.toString((int) (Math.random() * 36), 36));
		result.append(Integer.toString((int) (Math.random() * 36), 36)).append(Integer.toString((int) (Math.random() * 36), 36));
		Cookie token1 = new Cookie(TOKEN1, Base64.encodeBase64String(result.toString().getBytes()));
		token1.setPath("/");
		token1.setMaxAge(UserUtil.expiredSeconds);
		resp.addCookie(token1);
		String result2 = PasswordEncryptUtil.digest(result.toString(), "localhost", TOKEN2_ENCRYPT_TIMES);
		...
		UserUtil userUtil = SpringUtil.getBean("userUtil", UserUtil.class);
		userUtil.recordUser(id, result.toString());
	}
	...
	static String checkAndReturnToken(HttpServletRequest req) {
		...
		if (token2.equals(PasswordEncryptUtil.digest(token1, req.getHeader("Host"), TOKEN2_ENCRYPT_TIMES))) {
			return token1;
		}
		return null;
	}
	...
	public static void main(String[] args) {
		String token1 = new String(Base64.decodeBase64("ODN4eWplbHBhZHNmbmZlYnpnemxnaGJpZ2hoMQ".getBytes()));
