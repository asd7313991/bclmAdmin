package org.example.system.system.user;
...
import org.example.system.db.mysql.mapper.system.AdminUserMapper;
import org.example.system.db.mysql.mapper.system.UserPostMapper;
...
//        });
        ...
        // 插入用户
        AdminUserDO user =mapperFacade.map(reqVO,AdminUserDO.class);
        ...
        user.setSign(gensalt);
        user.setPassword(BCrypt.hashpw(reqVO.getPassword(),gensalt)); // 加密密码
    ...
    public void updateUser(UserUpdateReqVO reqVO) {
        ...
        validateUserForCreateOrUpdate(reqVO.getId(), reqVO.getUsername(), reqVO.getMobile(), reqVO.getEmail(),
                reqVO.getDeptId(), reqVO.getPostIds());
        // 更新用户
        AdminUserDO updateObj = mapperFacade.map(reqVO,AdminUserDO.class);
    ...
    public void updateUserProfile(Long id, UserProfileUpdateReqVO reqVO) {
        ...
        // 执行更新
        AdminUserDO convert = UserConvert.INSTANCE.convert(reqVO);
        convert.setId(id);
    ...
    public void updateUserPassword(Long id, UserProfileUpdatePasswordReqVO reqVO) {
        ...
        adminUserDO.setId(id);
        String gensalt = BCrypt.gensalt(10);
        adminUserDO.setSign(gensalt);
        adminUserDO.setPassword(BCrypt.hashpw(reqVO.getNewPassword(),gensalt)); // 加密密码
        userMapper.updateById(adminUserDO);
    }
...
//        String avatar = fileApi.createFile(IoUtil.readBytes(avatarFile));
    ...
    public void updateUserPassword(Long id, String password) {
        ...
        updateObj.setPassword(hashpw); // 加密密码
        updateObj.setSign(gensalt);
        userMapper.updateById(updateObj);
    }
    ...
    public UserImportRespVO importUserList(List<UserImportExcelVO> importUsers, boolean isUpdateSupport) {
        ...
        importUsers.forEach(importUser -> {
            ...
            if (existUser == null) {
                ...
                convert.setPassword(encodePassword(userInitPassword));
                convert.setPostIds(new HashSet<>());
                userMapper.insert(convert); // 设置默认密码及空岗位编号数组
