...
/**
 * The type Aspect log ext aop.
 */
@Slf4j
@Component
@Aspect
public class SystemAspectLogExtAop {
    private ExpressRunner expressRunner = new ExpressRunner();

    /**
     * Instantiates a new Aspect log ext aop.
     */
    public SystemAspectLogExtAop() {
    }
     ...
     * Convert.
     *
     * @param tlogAspect        the tlog aspect
    ...
    public void convert(TLogAspectExt tlogAspect, String method, Map<String, Object> paramNameValueMap, Object result, LocalDateTime startTime, Throwable exception) {
        ...
        try {
            log.info("开始记录操作日志，类型：{},方法：{},请求参数：{}，响应:{}，开始时间：{}", JSONUtil.toJsonStr(tlogAspect), method, JSONUtil.toJsonStr(paramNameValueMap), JSONUtil.toJsonStr(result), startTime);
        } catch (Exception exception1) {
            log.error("开始记录操作日志，类型：{},方法：{},请求参数：{}，开始时间：{},异常：{}", JSONUtil.toJsonStr(tlogAspect), method, JSONUtil.toJsonStr(paramNameValueMap), JSONUtil.toJsonStr(result), startTime, exception);
        }
        ...
        if (result instanceof CommonResult) {
            ...
            operateLogCreateReqDTO.setResultCode(SUCCESS.getResult());
        }
        ...
        if (StpSystemUtil.isLogin()) {
            // 补全用户信息
            operateLogCreateReqDTO.setUserId(StpSystemUtil.getLoginIdAsLong());
